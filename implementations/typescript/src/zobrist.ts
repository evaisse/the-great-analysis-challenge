...
export class ZobristKeys {
  ...
  computeHash(state: GameState): bigint {
    ...
    if (state.castlingRights.whiteKingside) hash ^= this.castling[0];
    if (state.castlingRights.whiteQueenside) hash ^= this.castling[1];
    if (state.castlingRights.blackKingside) hash ^= this.castling[2];
    if (state.castlingRights.blackQueenside) hash ^= this.castling[3];
