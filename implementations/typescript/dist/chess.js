...
const iterativeDeepening_1 = require("./iterativeDeepening");
class ChessEngine {
    constructor() {
    ...
    handleGo(args) {
        ...
        const timeManager = new timeManager_1.TimeManager(timeControl, moveNumber, isWhite);
        const result = (0, iterativeDeepening_1.iterativeDeepening)(this.board, maxDepth, timeManager, this.ai);
        if (!result.bestMove) {
            console.log("ERROR: No legal moves available");
            return;
        }
        const moveStr = this.board.squareToAlgebraic(result.bestMove.from) +
