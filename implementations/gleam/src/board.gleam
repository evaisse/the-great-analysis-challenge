...
pub fn set_piece(
  ...
  let new_board =
    list.index_map(game_state.board, fn(current_piece, index) {
      case index == square {
        True -> piece
        False -> current_piece
      }
...
pub fn make_move(game_state: GameState, chess_move: Move) -> GameState {
  ...
  case piece {
    ...
    Some(moving_piece) -> {
      ...
      let new_en_passant = case
        moving_piece.piece_type == Pawn
      ...
      let new_halfmove = case
        moving_piece.piece_type == Pawn || chess_move.captured != None
      {
        True -> 0
  ...
  case chess_move.is_castling {
    ...
    True -> {
      ...
      let #(rook_from, rook_to) = case chess_move.to == rank * 8 + 6 {
        True -> #(rank * 8 + 7, rank * 8 + 5)
        False -> #(rank * 8, rank * 8 + 3)
      }
  ...
  case chess_move.promotion {
    ...
    Some(promotion_type) ->
      set_piece(
        game_state,
        chess_move.to,
        Some(Piece(promotion_type, piece.color)),
      )
  }
}
...
fn update_castling_rights(
  ...
  // Also update when rook is captured
  let rights = case chess_move.to {
    0 -> CastlingRights(..rights, white_queenside: False)
    7 -> CastlingRights(..rights, white_kingside: False)
    56 -> CastlingRights(..rights, black_queenside: False)
    63 -> CastlingRights(..rights, black_kingside: False)
    _ -> rights
  }
        ...
        Some(moved_piece) -> {
          ...
          let state_after_undo =
            ...
            |> set_piece(last_move.to, case last_move.captured {
              Some(captured_type) ->
                Some(Piece(captured_type, opposite_color(moved_piece.color)))
              None -> None
            })

          // Handle en passant undo
          let state_after_undo = case last_move.is_en_passant {
            True -> {
              let captured_pawn_square = case moved_piece.color {
                White -> last_move.to - 8
                Black -> last_move.to + 8
              }
              set_piece(
                state_after_undo,
                captured_pawn_square,
                Some(Piece(Pawn, opposite_color(moved_piece.color))),
              )
            }
            False -> state_after_undo
          }

          // Handle castling undo
          let state_after_undo = case last_move.is_castling {
            True -> {
              let rank = case moved_piece.color {
                White -> 0
                Black -> 7
              }
              let #(rook_from, rook_to) = case last_move.to == rank * 8 + 6 {
                True -> #(rank * 8 + 7, rank * 8 + 5)
                False -> #(rank * 8, rank * 8 + 3)
              }
              state_after_undo
              |> set_piece(rook_from, get_piece(state_after_undo, rook_to))
              |> set_piece(rook_to, None)
            }
            False -> state_after_undo
          }
