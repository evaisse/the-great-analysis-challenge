...
module Chess
  class AI
    ...
    def minimax(depth, alpha, beta, maximizing_player, color)
      ...
      return STALEMATE_VALUE if @move_generator.in_stalemate?(color)
    ...
    def evaluate_position_bonuses(perspective_color)
      ...
      if king_pos
        ...
        bonus -= 20 if king_exposed?(king_row, king_col, perspective_color)
      end
      ...
      if enemy_king_pos
        ...
        bonus += 20 if king_exposed?(enemy_king_row, enemy_king_col, perspective_color == :white ? :black : :white)
      end

      bonus += endgame_king_distance_bonus

      bonus
    end

    def endgame_king_distance_bonus
      return 0 unless simplified_endgame?

      white_king_pos = @board.find_king(:white)
      black_king_pos = @board.find_king(:black)
      return 0 unless white_king_pos && black_king_pos

      white_row, white_col = white_king_pos
      black_row, black_col = black_king_pos
      14 - AttackTables.manhattan_distance(white_row, white_col, black_row, black_col)
    end

    def simplified_endgame?
      non_pawn_pieces = 0
      queen_count = 0
      ...
      (0..7).each do |row|
        (0..7).each do |col|
          ...
          non_pawn_pieces += 1 unless %i[pawn king].include?(piece.type)
        end
      end

      non_pawn_pieces <= 4 || (non_pawn_pieces <= 6 && queen_count.zero?)
    end
