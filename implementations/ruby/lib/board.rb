...
module Chess
  class Board
    ...
    def make_move(move)
      ...
      # 2. Handle capture
      hash ^= ZOBRIST.pieces[ZOBRIST.piece_index(target_piece)][((7 - move.to_row) * 8) + move.to_col] if target_piece
      ...
      # 3. Handle promotion
      final_piece = piece
      final_piece = Piece.new(move.promotion, piece.color) if move.promotion && piece.type == :pawn
    ...
    def undo_move(move)
      ...
      set_piece(move.to_row, move.to_col, move.captured_piece) if move.captured_piece && !move.is_en_passant
    ...
    def update_castling_rights(move, piece)
      ...
      return unless (move.from_row == 0 && move.from_col == 7) || (move.to_row == 0 && move.to_col == 7)
    ...
    def can_knight_attack?(from_row, from_col, to_row, to_col)
      AttackTables.knight_attacks(from_row, from_col).include?([to_row, to_col])
    end

    def can_bishop_attack?(from_row, from_col, to_row, to_col)
      ray_attacks_square?(to_row, to_col, AttackTables.bishop_rays(from_row, from_col))
    end

    def can_rook_attack?(from_row, from_col, to_row, to_col)
      ray_attacks_square?(to_row, to_col, AttackTables.rook_rays(from_row, from_col))
    end

    def can_queen_attack?(from_row, from_col, to_row, to_col)
      ray_attacks_square?(to_row, to_col, AttackTables.queen_rays(from_row, from_col))
    end

    def can_king_attack?(from_row, from_col, to_row, to_col)
      AttackTables.king_attacks(from_row, from_col).include?([to_row, to_col])
    end

    def ray_attacks_square?(to_row, to_col, rays)
      rays.any? do |ray|
        ray.each do |square_row, square_col|
          return true if square_row == to_row && square_col == to_col
          break if piece_at(square_row, square_col)
        end
