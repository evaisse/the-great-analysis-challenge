...
module Chess
  class Board
    ...
    def update_castling_rights(move, piece)
      ...
      @castling_rights.white_queenside = false if (move.from_row == 7 && move.from_col.zero?) || (move.to_row == 7 && move.to_col.zero?)
      @castling_rights.white_kingside = false if (move.from_row == 7 && move.from_col == 7) || (move.to_row == 7 && move.to_col == 7)
      @castling_rights.black_queenside = false if (move.from_row.zero? && move.from_col.zero?) || (move.to_row.zero? && move.to_col.zero?)
      return unless (move.from_row.zero? && move.from_col == 7) || (move.to_row.zero? && move.to_col == 7)
