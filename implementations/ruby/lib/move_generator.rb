  ...
  class MoveGenerator
    ...
    def generate_pawn_moves(color, row, col)
      ...
      [-1, 1].each do |col_offset|
        ...
        # En passant capture
        next unless @board.en_passant_target &&
    ...
    def generate_knight_moves(row, col)
      ...
      AttackTables.knight_attacks(row, col).each do |new_row, new_col|
        target_piece = @board.piece_at(new_row, new_col)

        moves << Move.new(row, col, new_row, new_col) if !target_piece || target_piece.color != piece.color
      end
    ...
    def generate_bishop_moves(row, col)
      generate_sliding_moves(row, col, AttackTables.bishop_rays(row, col))
    end

    def generate_rook_moves(row, col)
      generate_sliding_moves(row, col, AttackTables.rook_rays(row, col))
    end

    def generate_queen_moves(row, col)
      generate_sliding_moves(row, col, AttackTables.queen_rays(row, col))
    end
    ...
    def generate_king_moves(row, col)
      ...
      AttackTables.king_attacks(row, col).each do |new_row, new_col|
        target_piece = @board.piece_at(new_row, new_col)
    ...
    def generate_sliding_moves(row, col, rays)
      moves = []
      piece = @board.piece_at(row, col)

      rays.each do |ray|
        ray.each do |new_row, new_col|
          target_piece = @board.piece_at(new_row, new_col)

          if target_piece
            moves << Move.new(row, col, new_row, new_col) if target_piece.color != piece.color
            break
          else
            moves << Move.new(row, col, new_row, new_col)
          end
    ...
    def generate_castling_moves(color)
      ...
      if (color == :white ? @board.castling_rights.white_kingside : @board.castling_rights.black_kingside) &&
         can_castle_kingside?(color, king_row)
      ...
      if (color == :white ? @board.castling_rights.white_queenside : @board.castling_rights.black_queenside) &&
         can_castle_queenside?(color, king_row)
